	...
	function init() {
				...
				for (i = 0; i < _links.length; i++) {
					...
					if (this.links.map(function(link) {
						...
						var sourceIndex = this.nodes.map(function(node) {
							return node.id;
						}).indexOf(_links[i].source);
						var targetIndex = this.nodes.map(function(node) {
							return node.id;
						}).indexOf(_links[i].target);
		...
		function processIncomingData(data) {
			...
			for (var i = 0; i < nodes.length; i++) {
				nodes[i].image = createHTMLImage(nodes[i].image, nodes[i].id);
			}
		...
		var getSubGraph = function(artist) {
			...
			// Get Graph
			$.ajax({
				url : encodeURI("/api/getGraph/" + lowerArtist),
				method : "GET",
        ...
        var getGraphFromLastFM = function(artist) {
            ...
            // Get Graph
            $.ajax({
                url : encodeURI("/2.0/?method=artist.getsimilar&artist=" + lowerArtist + "&api_key=ecc2e1fc920c9fcbcc19ba8790570691&format=json"),
                method : "GET",
                success : function(data) {
                    processIncomingData(data);
                }
		...
		return {
            ...
            getGraphFromLastFM : function(artist) {
                var lowerArtist = artist.toLowerCase();
